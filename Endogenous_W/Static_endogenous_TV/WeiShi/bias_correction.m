...
function [bc,sd,gamma_z,f_z,gamma_y,f_y,C,SIG] = bias_correction(theta,n,T,Rz,Ry,W,X,Y,Z,X_Z,info,biasopt)
...
est = [beta_z;beta_y;lambda;alpha_xi;alpha;delta];
...
%% Projection matrices
%% Gamma_z
M = (1/sigma_e)*(Z-X_Z*kron(beta_z,eye(T)));
[U,D,~] = svd(M);
D = diag(D).*diag(D);
...
for t = 1:T
    S = eye(n)-lambda*W(:,:,t);
    G(:,t) = (1/sigma_xi)*(S*Y(:,t)-cX_Y(:,t)-(cZ(:,t))*delta);
end
[U,D,~] = svd(G);
D = diag(D).*diag(D);
[~,I] = sort(D);
...
%% Matrix C
% SIGMA matrix
SIG = zeros(kz+ky+2+1+1); % kz+ky+2+J+p

% Elements corresponding to \beta_z\beta_z
for k1 = 1:kz
    X_z_k1 = X_Z(:,T*(k1-1)+1:k1*T);
    for k2 = 1:kz
        X_z_k2 = X_Z(:,T*(k2-1)+1:k2*T);
        SIG(k1,k2) = trace(M_Fz*X_z_k1'*M_Gz*X_z_k2)/(n*T*sigma_e^2) + ...
             trace(M_Fy*X_z_k1'*delta*M_Gy*delta*X_z_k2)/(n*T*sigma_xi^2);
    end
end
...
for k1 = 1:kz
    ...
    for k2 = 1:ky
        ...
        SIG(kz+k2,k1) = -trace(M_Fy*X_z_k1'*delta*M_Gy*X_y_k2)/...
                                                        (n*T*sigma_xi^2);
        SIG(k1,kz+k2) = -trace(M_Fy*X_z_k1'*delta*M_Gy*X_y_k2)/...
                                                        (n*T*sigma_xi^2);
    end
end
...
for k1 = 1:kz
    X_z_k1 = X_Z(:,T*(k1-1)+1:k1*T);
    SIG(kz+ky+1,k1) = -trace(M_Fy*X_z_k1'*delta*M_Gy*X2)/(n*T*sigma_xi^2);
    SIG(k1,kz+ky+1) = -trace(M_Fy*X_z_k1'*delta*M_Gy*X2)/(n*T*sigma_xi^2);
end
...
for k1 = 1:kz
    SIG(kz+ky+2,k1) = 0;
    SIG(k1,kz+ky+2) = 0;
    SIG(kz+ky+3,k1) = 0;  % These two last lines will change in terms of dimension
    SIG(k1,kz+ky+3) = 0;
end
...
for k1 = 1:ky
    SIG(kz+ky+2,kz+k1) = 0;
    SIG(kz+k1,kz+ky+2) = 0;
    SIG(kz+ky+3,kz+k1) = 0;  % These two last lines will change in terms of dimension
    SIG(kz+k1,kz+ky+3) = 0;
end
...
SIG(kz+ky+3,kz+ky+1) = 0;  % These two last lines will change in terms of dimension
SIG(kz+ky+1,kz+ky+3) = 0;

% Elements corresponding to \sigma_xi\alpha, \sigma_xi\sigma_xi and \alpha\alpha
SIG(kz+ky+2,kz+ky+3) = 0;
SIG(kz+ky+3,kz+ky+2) = 0;
SIG(kz+ky+2,kz+ky+2) = 2*sigma_xi^2;
SIG(kz+ky+3,kz+ky+3) = 2*sigma_e^2; 

% Elements corresponding to \delta\beta_z
% These lines will change in terms of dimension
for k1 = 1:kz
    X_z_k1 = X_Z(:,T*(k1-1)+1:k1*T);
    SIG(k1,kz+ky+4) = -trace(M_Gy*delta*X_z_k1*M_Fy*...
                           (Z-X_Z*kron(beta_z,eye(T)))')/(n*T*sigma_xi^2);
    SIG(kz+ky+4,k1) = -trace(M_Gy*delta*X_z_k1*M_Fy*...
                           (Z-X_Z*kron(beta_z,eye(T)))')/(n*T*sigma_xi^2);
end
...
for k1 = 1:ky
    X_y_k1 = X(:,T*(k1-1)+1:k1*T);
    SIG(kz+k1,kz+ky+4) = -trace(M_Gy*X_y_k1*M_Fy*(Z-X_Z*kron(beta_z,eye(T)))')...
                                                         /(n*T*sigma_xi^2);
    SIG(kz+ky+4,kz+k1) = -trace(M_Gy*X_y_k1*M_Fy*(Z-X_Z*kron(beta_z,eye(T)))')...
                                                         /(n*T*sigma_xi^2);
end
...
SIG(kz+ky+1,kz+ky+4) = -trace(M_Gy*X2*M_Fy*(Z-X_Z*kron(beta_z,eye(T)))')...
                                                         /(n*T*sigma_xi^2);
SIG(kz+ky+4,kz+ky+1) = -trace(M_Gy*X2*M_Fy*(Z-X_Z*kron(beta_z,eye(T)))')...
                                                         /(n*T*sigma_xi^2);
% Elements corresponding to \delta\sigma_xi
% These lines will change in terms of dimension
SIG(kz+ky+2,kz+ky+4) = 0;
SIG(kz+ky+4,kz+ky+2) = 0;

% Elements corresponding to \delta\alpha
% These lines will change in terms of dimension
SIG(kz+ky+3,kz+ky+4) = 0;
SIG(kz+ky+4,kz+ky+3) = 0;

% Elements corresponding to \alpha\alpha
% These lines will change in terms of dimension
SIG(kz+ky+4,kz+ky+4) = trace(M_Gy*(Z-X_Z*kron(beta_z,eye(T)))*...
                       M_Fy*(Z-X_Z*kron(beta_z,eye(T)))')/(n*T*sigma_xi^2);
...
%%%%%%
 % factor loadings
    ...
    f_y = zeros(T,Ry);
    for t = 1:T
        res = (eye(n)-lambda*squeeze(W(:,:,t)))*Y(:,t)- cX_Y(:,t)-(cZ(:,t))*delta;
        f_y(t,:) = ((gamma_y'*gamma_y)\(gamma_y'*res))'; 
    end
    f_z = ((gamma_z'*gamma_z)\(gamma_z'*cZ))'; 
