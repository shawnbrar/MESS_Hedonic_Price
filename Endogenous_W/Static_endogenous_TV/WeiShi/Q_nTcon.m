...
function [f] = Q_nTcon(theta,n,T,Rz,Ry,W,x,y,z,x_z,info)
...
%  In the paper, we need to compute the eigenvalues of DD' and not of D. I
...
if sum(sum(isnan(D))) ~= 0 || sum(sum(isinf(D))) ~= 0 ||...
    ...
    ll1 = ll-(1/2)*sum(D1(Rz+1:end))/(n*T)-(1/2)*sum(G1(Ry+1:end))/(n*T);
    f = -ll1;
