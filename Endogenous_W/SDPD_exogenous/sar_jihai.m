...
for i=1:nf,
    if strcmp(fields{i},'tl')       
        tl = info.tl; % tar lag index temporal lag 
    elseif strcmp(fields{i},'stl')
        stl = info.stl; % star lag index spatio-temporal lag
...
% step 2) maximize concentrated likelihood function;
    ...
    f_sar_jihai_oc=@(x) f_sar_jihai(x,detval,epe0,eped,epe0d,n,t);
    [p,liktmp,exitflag,output] = fminbnd(f_sar_jihai_oc,rmin,rmax,options);
...
if m==n,
    Bn=An;  %%%%%%%%%%%%%%%%this does not happen in practise
end
...
bvec2 = theta1;
tmps2 = [bvec2./(sqrt(tmpplus2))]*sqrt(n*t);
results.tstat2 = tmps2;
results.std2=sqrt(tmpplus2)/sqrt(n*t);
...
parm1 = results.theta1;

info_f2=struct('tl',tl,'stl',stl);
        
results.lik1 = f2_sar_jihai(parm1,y,x,W,detval,info_f2);
