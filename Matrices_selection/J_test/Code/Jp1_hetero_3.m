function [stat1, stat2,stat3,signi1]=Jp1_hetero_3(y,W_1,W_2,W_3,x,res_1,res_2,res_3,B)
% This code is for the first predictor.
% So it's just an GMM to instrument the Wy using the W which is assumed the
% true one
rho_w1=res_1.rho;
beta_w1=res_1.beta;
res_w1=res_1.resid;
...
parfor b=1:B
   ...
   w2ys=W_2*ystar;
   w3ys=W_3*ystar;
               RGMM2st=gmm_sar_hetero_mc_old(ystar,x, W_2,w2ys,Q2,p2,par_ini_n,Wen);
               RGMM3st=gmm_sar_hetero_mc_old(ystar,x, W_3,w3ys,Q3,p3,par_ini_n,Wen);
            rho_w2s=RGMM2st.par(1,1);
            beta_w2s=RGMM2st.par(2:end,1);
            A2is=In/(In-rho_w2s*W_2);
            y2ps=A2is*(x*beta_w2s);
            
            rho_w3s=RGMM3st.par(1,1);
            beta_w3s=RGMM3st.par(2:end,1);
            A3is=In/(In-rho_w3s*W_3);
            y3ps=A3is*(x*beta_w3s);
...
% For model 2
            ...
            w1ys=W_1*ystar;
            w3ys=W_3*ystar;
            RGMM1st=gmm_sar_hetero_mc_old(ystar,x, W_1,w1ys,Q1,p1,par_ini_n,Wen);
            rho_w1s=RGMM1st.par(1,1);
            beta_w1s=RGMM1st.par(2:end,1);
            
            A1is=In/(In-rho_w1s*W_1);
            y1ps=A1is*(x*beta_w1s);
