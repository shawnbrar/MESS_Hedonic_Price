function prt_sar_conv_g(results,vnames,fid,fmt)
...
if ~isstruct(results)
 error('prt_sar_convg requires structure argument');
elseif nargin == 1
...
elseif nargin == 4
 ...
 error('Wrong # of arguments to prt_sar_convg');
end;
...
if m>1 
    ...
    [tst_n nsize] = size(vnames);
     if tst_n ~= nvar+1
     fprintf(fid,'Wrong # of variable names in prt_sar_convg -- check vnames argument \n');
    ...
    vnameso = strvcat(Vname(3:end-1-m,:));
    elseif cflag == 1
    vnameso = strvcat(Vname(2:end-1-m,:));
    end    
    ...
    mprint(printout,ini);
%% Case with only 1 single weight matrix
else 
    if (nflag == 1) % the user supplied variable names
    [tst_n nsize] = size(vnames);
     if tst_n ~= nvar+1
     fprintf(fid,'Wrong # of variable names in prt_sar_conv -- check vnames argument \n');
     fprintf(fid,'will use generic variable names \n');
     nflag = 0;
     end
    end;
...
%                 gout];
    ...
    nd = (results.ndraw-results.nomit)/results.thin;
    fprintf(fid,'      MCMC diagnostics ndraws = %d \n',nd);
    cstats=chainstats([results.bdraw results.pdraw]);
    in.cnames = strvcat('mode','mean','MC error','tau','Geweke');
    in.rnames = Vname;
    in.fmt = strvcat('%12.4f','%12.4f','12.8f','%10.6f','%10.6f');
    in.width = 10000;
    pmode = [results.beta_mode results.rho_mode]';
    out = [pmode cstats];
    mprint(out,in);
    ...
    % % column labels for printing results
    cnames = strvcat('lower 0.005','lower 0.025','median','upper 0.975','upper 0.995');
    in.cnames = cnames;
    in.rnames = Vname;
    ...
    mprint(printout,ini);   
    fprintf(fid,'***************************************************************\n');
    fprintf(fid,' Attention, only one connectivity matrix is present in the model \n');
