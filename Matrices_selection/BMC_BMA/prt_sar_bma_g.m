function prt_sar_bma_g(results,vnames,fid,fmt)
...
if ~isstruct(results)
 error('prt_sar_bma_g requires structure argument');
elseif nargin == 1
...
elseif nargin == 4
 ...
 error('Wrong # of arguments to prt_sar_bma_g');
end;
...
nobs = results.nobs;
cflag = results.cflag;
nmat = results.np; % Number of considered connectivy matrices.

if (nflag == 1) % the user supplied variable names
[tst_n nsize] = size(vnames);
 if tst_n ~= nvar+1
 fprintf(fid,'Wrong # of variable names in prt_sar_bma_g-- check vnames argument \n');
...
if nflag == 0 % no user-supplied vnames or an incorrect vnames argument
    ...
    elseif cflag == 1 % no constant term
