...
% step 2) maximize concentrated likelihood function;
    ...
    f_sarpanel_oc=@(x) f_sarpanel(x,detval,epe0,eped,epe0d,N,T);
    [p,liktmp,exitflag,output] = fminbnd(f_sarpanel_oc,rmin,rmax,options);
...
sige = (1/(nobs))*(e0-p*ed)'*(e0-p*ed);    
if model==1
 sige = (1/(nobs-N))*(e0-p*ed)'*(e0-p*ed);
end
 % Attention : Correction only valid for model=1
results.sige = sige;
...
t0 = clock;
f2_sarpanel_oc=@(x) f2_sarpanel(x,ywith,xwith,W,detval,T);
dhessn = hessian(f2_sarpanel_oc,parm); %Elhorst
hessi = invpd(-dhessn);
