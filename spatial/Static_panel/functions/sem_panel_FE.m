...
e = (ywith - xwith*b);
rold = rho;
f_sempanel_oc=@(x) f_sempanel(x,e,W,detval,T);
[rho,like,exitflag,output] = fminbnd(f_sempanel_oc,rmin,rmax,options);
...
t0 = clock;
f2_sempanel_oc=@(x) f2_sempanel(x,ywith,xwith,W,detval,T);
hessn = hessian(f2_sempanel_oc,parm); %Elhorst
