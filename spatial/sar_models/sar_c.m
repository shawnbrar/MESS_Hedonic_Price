...
detval=a;
logm_sar_profile = -dof*log(epe0*iotan - 2*xx*epe0d + (xx.*xx)*eped) + detval;
[adj,mind] = max(logm_sar_profile);


results.maxr_sar = xx(mind);
...
logm_out = isum + logC_sar; % we put back the scale adjustment here
% 
results.logmarginal = logm_out;
...
rmin = -0.999;     % use -1,1 rho interval as default
rmax = 0.999;
