function prt_sar_g(results,vnames,fid,fmt)
% PURPOSE: Prints output using SAR results structures
%---------------------------------------------------
% USAGE: prt_sar_g(results,vnames,fid)
...
% --------------------------------------------------
%  RETURNS: nothing, just prints the sar_gnew estimation results
% --------------------------------------------------
% SEE ALSO: prt, plt
%---------------------------------------------------   
...
if ~isstruct(results)
 error('prt_sar_g requires structure argument');
elseif nargin == 1
...
elseif nargin == 4
 ...
 error('Wrong # of arguments to prt_sar_g');
end;
...
[tst_n nsize] = size(vnames);
 if tst_n ~= nvar+1
 fprintf(fid,'Wrong # of variable names in prt_sar_g -- check vnames argument \n');
...
fprintf(fid,'time for traces     =  %9.4f \n',results.trace_time);
fprintf(fid,'thinning for draws  = %9d   \n',results.thin);
fprintf(fid,'min and max rho     = %9.4f,%9.4f \n',results.rmin,results.rmax);
if results.taylor == 1
    fprintf(fid,'Taylor series used \n');
elseif results.cheby == 1
    fprintf(fid,'Chebyshev series used \n');
end;
...
in.width = 10000;
pmode = [results.posterior_mode(1,2:end-1) results.posterior_mode(1,1)]';
out = [pmode cstats];
mprint(out,in);
