...
% In this code, you don' need to specify info.lflag=0 because the code does
% not use any approximation for the computation of the Jacobian of the
% transformation. It uses the eigenvalues of the W matrix. 
...
%      Use of LeSage approximation to compute Jacobian
f_sar_oc=@(x) f_sar(x,detval,epe0,eped,epe0d,n);
 [prho,liktmp,exitflag,output] = fminbnd(f_sar_oc,rmin,rmax,options);
%    [prho,liktmp,exitflag,output] = fminbnd('f_sar_mod',rmin,rmax,options,W,epe0,eped,epe0d,n); %Here we use eigenvalues to compute th
...
% Nez residual, comes from the implicit form. 
results.resid1 = y -prho*W*y-x*bhat;
% New fitted values from the implicit form.
results.yhat1=y-results.resid1;
...
% results.lik = f2_sar_mod(parm,y,x,W);
if n <= 2000
t0 = clock;
...
hessi1=hessi(1:nvar+1,1:nvar+1)-diag(diag(hessi(1:nvar+1,1:nvar+1)))+diag(tmp);
results.cov=hessi1;
bvec = [results.beta
        results.rho];
tmps = bvec./(sqrt(tmp));
...
tvar = abs(diag(hessi));
xpxi = hessi - diag(diag(hessi))+diag(tvar);
results.cov=xpxi;
tmp = [results.beta
       results.rho];
...
%  Steps until lines 270 serve to compute direct effects estimate. It mainly consists
% in computing traces averages of the geometric expansion up to order 100. 
%  To do so, Jim uses its efficient computation routines based on p.112
%  expression 4.131 of his book. 
uiter=50;
maxorderu=100; % Order considered in the geometric expansion
nobs = n;
rv=randn(nobs,uiter);
...
traces=[tracew];
traces(1)=0; %Trace of W
traces(2)=sum(sum(W'.*W))/nobs; %average of Trace of W^2
trs=[1;traces]; % The first element is the average of the trace of I
ntrs=length(trs);
trbig=trs'; % This is what will be used to compute direct effects. It corresponds to expression 4.144 of Jim's book.
...
tmp = [results.beta
       results.rho];
% Draw from a multivariate normal distribution (where sigmatt has been transformed to avoid negative variance (see line  277)  
...
for i=1:ndraw;
    rmat = pdraws(i,1).^ree; % represents power of rho (it's the "g" in equation 4.145 of the book)
    for j=1:p;
            beta = [bdraws(i,j)];
            total(i,j,:) = beta(1,1)*rmat; % This comes from the fact that W is row-stochastic (see equation 2.47 of the book)
...
%         w = eigenvalues of the interactions matrix.
%         epe0   = see below
%         eped   = see below
%         eoe0d  = see below
%         n      = # of obs
...
function llike = f_sar_mod(rho,W,epe0,eped,epe0d,n)
% PURPOSE: evaluates concentrated log-likelihood for the 
%  spatial autoregressive model using sparse matrix algorithms
% ---------------------------------------------------
%  USAGE:llike = f_sar(rho,detval,epe0,eped,epe0d,n)
...
llike = (n/2)*log(z) - sum1;


function llike = f2_sar_mod(parm,y,x,W)
...
epe = e'*e;
tmp2 = 1/(2*sige);
llike = -(n/2)*log(pi) - (n/2)*log(sige) + sum1 - tmp2*epe;
